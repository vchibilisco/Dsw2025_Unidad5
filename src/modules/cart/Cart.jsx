...
// --- Componente para una fila del carrito ---
...
const LoginModal = ({ onClose }) => {
  const navigate = useNavigate(); // Usamos la función de autenticación real del contexto
  ...
  const onSubmit = async (data) => {
    // 1. Llamada REAL al login
    const { error } = await singin(data.email, data.password);
    if (!error) {
      // Login exitoso
      onClose();
      navigate("/checkout"); // Redirige al checkout para ingresar las direcciones.
    } else {
...
const Cart = () => {
  ...
  const handleCheckout = () => {
    ...
    // user será null si no está logueado
    if (!user) {
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-100 block sm:table-row-group">
                ...
                {cartItems.map((item) => (
                  <CartItemRow
                    key={item.id}
                    item={item} // PASAMOS LAS FUNCIONES OBTENIDAS POR useCart()
